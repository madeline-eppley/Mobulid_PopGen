...
new parameter opt script that uses p=1 when optimizing the catalog building, then a final run that uses populations-1=p to be just slightly less stringent on the filtering and preserve alleles that are unique to each population. since all of the tarapacana samples are high coverage (greater than 10x), i did not do a separate highcov/lowcov step in this script, the catalog was simply built using all samples. 
